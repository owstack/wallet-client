 ...
 * parseSecret(secret)
 * _buildSecret(walletId, walletPrivKey, networkName)
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (!self.credentials) {
    self.ctx.log.info('Generating new keys for ' + networkName);
    self.seedFromRandom({
  ...
  self._doPostRequest('/v1/wallets/', args, function(err, res) {
    ...
    var secret = self._buildSecret(c.walletId, c.walletPrivKey, c.networkName);
  ...
  self.getStatus({
    ...
    self._doPostRequest('/v1/wallets/', args, function(err, body) {
      ...
      async.each(self.credentials.publicKeyRing, function(item, next) {
        ...
        self._doJoinWallet(walletId, walletPrivKey, item.xPubKey, item.requestPubKey || item.requestPubKeys[0].key, name, {
          supportBIP44AndP2PKH: supportBIP44AndP2PKH,
        }, function(err) {
...
API.prototype.parseSecret = function(secret) {
  ...
  try {
    ...
    var walletPrivKey = PrivateKey.fromString(secretSplit[1], networkName);
...
API.prototype._buildSecret = function(walletId, walletPrivKey, networkName) {
  var self = this;
  if (lodash.isString(walletPrivKey)) {
    walletPrivKey = PrivateKey.fromString(walletPrivKey, networkName);
  }
  ...
  return lodash.padEnd(widBase58, 22, '0') + walletPrivKey.toWIF() + networkNameBase58;
};
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  self.getUtxos({
    ...
    return cb(null, lodash.sumBy(utxos, self.Unit().atomicsName()), address.toString());
  });
};
...
API.prototype.getStatus = function(opts, cb) {
  ...
  self._doGetRequest('/v1/wallets/?' + qs.join('&'), function(err, result) {
    ...
    if (result.wallet.status == 'pending') {
      var c = self.credentials;
      result.wallet.secret = self._buildSecret(c.walletId, c.walletPrivKey, c.networkName);
    }
...
API.prototype.buildTx = function(txp) {
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    lodash.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        ...
        var out = {};
        out.script = o.script;
        out[self.Unit().atomicsName()] = o.amount;
  ...
  var totalInputs = lodash.reduce(txp.inputs, function(memo, i) {
    return +i[self.Unit().atomicsName()] + memo;
  }, 0);
  var totalOutputs = lodash.reduce(t.outputs, function(memo, o) {
    return +o[self.Unit().atomicsName()] + memo;
  }, 0);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var amount = lodash.sumBy(utxos, self.Unit().atomicsName()) - fee;
      if (amount <= 0) {
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  headers['x-service'] = this.LIVENET.protocol; // Livenet protocol is used to id the service
