...
function API(context, opts) {
  ...
  this.CURRENCY_CODE = this.ctx.Networks.livenet.code; // Always based on livenet
...
API.prototype.seedFromRandom = function(opts) {
  opts = opts || {};
//  this.credentials = Credentials.fromRandom(opts.network || Constants.LIVENET);
console.log('###### '+opts.network || this.LIVENET);
  this.credentials = Credentials.fromRandom({
    network: opts.network || this.LIVENET
  });
/*
  this.credentials = Credentials.fromRandom({
    network: opts.network || Constants.LIVENET
  });
*/
console.log('CREDS');
//console.log(opts.network || Constants.LIVENET);
//console.log(Utils.translateNetwork(opts.network || Constants.LIVENET));
  this.credentials = Credentials.fromRandom({
//    network: this.ctx.networkToSymbol(opts.network || Constants.LIVENET)
    network: this.ctx.Networks.get(opts.network || this.LIVENET).code

  });
  
//  this.credentials.network = this.ctx.symbolToNetwork(this.credentials.network);
//  this.credentials.network = this.ctx.Networks.get(this.credentials.network).code;
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  opts.network = opts.network || this.LIVENET;
  opts.langauge = opts.language || 'en';
  opts.account = opts.account || 0;

  this.credentials = Credentials.fromRandomMnemonic(opts);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || this.LIVENET, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.network || this.LIVENET, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  return PrivateKey.decryptBIP38PrivateKey(encryptedPrivateKeyBase58, passphrase, opts, cb);
};
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = this.ctx.Address.fromString(destinationAddress);

        tx = new this.ctx.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
console.log('API.prototype.openWallet '+JSON.stringify(self.credentials));      
console.log('API.prototype.openWallet '+JSON.stringify(wallet.copayers));      
...
console.log('secretSplit '+secretSplit);
...
console.log('2');
    var network = (networkChar == 'l' ? this.LIVENET : this.TESTNET);
console.log('3 '+network);
    var currency = secretSplit[3];

console.log('network '+network);
    var walletPrivKey = PrivateKey.fromString(secretSplit[1], network);

console.log('currency '+currency);
console.log('this.CURRENCY_CODE '+this.CURRENCY_CODE);
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || lodash.includes([this.LIVENET, this.TESTNET], network));
  network = network || this.LIVENET;
...
console.log(this.LIVENET);

//  if (!lodash.includes([Constants.TESTNET, Constants.LIVENET], network)) {
  if (!lodash.includes([this.ctx.Networks.livenet.code, this.ctx.Networks.testnet.code], network)) {
    return cb(new Error('Invalid network'));
  }

  if (!self.credentials) {
    log.info('Generating new keys for ' + network);
    console.log('Generating new keys for ' + network);
    self.seedFromRandom({
      network: network
    });
  } else {
...
API.prototype.getPreferences = function(cb) {
  ...
  this._doGetRequest('/v1/preferences/', function(err, preferences) {
    if (err) {
      return cb(err);
    }
...
API.prototype.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      address = this.ctx.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(lodash.isArray(publicKeys) && publicKeys.length == 1);
      address = this.ctx.Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
API.prototype.signTxProposalFromAirGappedWithNewClient = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    if (key.slice(0, 4) === 'xprv' && txp.network == this.TESTNET) {
      throw new Error("testnet HD keys must start with tprv");
    }
    if (key.slice(0, 4) === 'tprv' && txp.network == this.LIVENET) {
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new HDPrivateKey(this.credentials.xPrivKey).deriveChild(this.credentials.getBaseAddressDerivationPath());
...
//  $.checkArgument(lodash.includes(lodash.keys(Constants.UNITS), unit));
  var units = lodash.map(this.ctx.Units.getUnits(), function(u) {
    return u.code;
  });
  $.checkArgument(units, unit);
...
//  var u = Constants.UNITS[unit];
  var u = lodash.find(this.ctx.Units.getUnits(), function(u) {
    return u.code == unit;
  });
...
//  var amount = clipDecimals((satoshis / u.toSatoshis), u[precision].maxDecimals).toFixed(u[precision].maxDecimals);
  var amount = clipDecimals((satoshis / u.value), u[precision].maxDecimals).toFixed(u[precision].maxDecimals);
...
API.prototype._buildSecret = function(walletId, walletPrivKey, network) {
  ...
  return lodash.padEnd(widBase58, 22, '0') +
    walletPrivKey.toWIF() +
//    (network == Constants.LIVENET ? 'l' : 't') +
    (network == this.LIVENET ? 'l' : 't') +
    this.CURRENCY_CODE;
};
