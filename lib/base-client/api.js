 ...
 *
 * @param {string} network - LIVENET.name (default) or TESTNET.name
 * @param {Callback} cb
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || lodash.includes([self.LIVENET.name, self.TESTNET.name], network));
  network = network || self.LIVENET.name;
...
console.log('SENDING *** ',method,url,JSON.stringify(args));
  if (args) {
...
console.log('RECEIVING *** ',method, url,JSON.stringify(res.body));
