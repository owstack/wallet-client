...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  var self = this;
  self.log.info('Importing from mnemonic');
...
API.prototype.importFromExtendedPrivateKey = function(xPrivKey, opts, cb) {
  var self = this;
  self.log.info('Importing from extended private key');
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  self.log.debug('Request:', method, self.baseUrl + url, headers);
  ...
  r.end(function(err, res) {
    ...
    if (!lodash.isEmpty(res.body)) {
      self.log.debug('Response:', util.inspect(res.body, {
        depth: 10
      }));
    }
    ...
    if (!lodash.inRange(res.status, 199, 300)) {
      ...
      if (lodash.inRange(res.status, 499, 600)) {
        return cb(new Errors.INTERNAL_ERROR(res.statusText || (res.body ? res.body.error : '')));
      }
      ...
      self.log.error('HTTP Error: ' + res.status + ' ' +
        res.req.method + ' ' +
        res.req.url + ' - [' + self.LIVENET.currency + ' client] ' + (res.body ? res.body.error : ''));
