...
//    network: this.ctx.networkToSymbol(opts.network || Constants.LIVENET)
    network: this.ctx.Networks.get(opts.network || this.ctx.Networks.livenet).alias
...
API.prototype.parseSecret = function(secret) {
  ...
  try {
    ...
    var walletPrivKey = PrivateKey.fromString(secretSplit[1]);
    var networkChar = secretSplit[2];
    var currencyChar = secretSplit[3];

    if (currencyChar != this.CURRENCY_CODE) {
    ...
    return {
      ...
      network: networkChar == 'l' ? Constants.LIVENET : Constants.TESTNET,
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || lodash.includes([Constants.LIVENET, Constants.TESTNET], network));
...
console.log(network);
console.log(Constants.TESTNET);
console.log(Constants.LIVENET);
...
API.prototype.signTxProposalFromAirGappedWithNewClient = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    if (key.slice(0, 4) === 'xprv' && txp.network == Constants.TESTNET) {
      throw new Error("testnet HD keys must start with tprv");
    }
    if (key.slice(0, 4) === 'tprv' && txp.network == Constants.LIVENET) {
...
API.prototype._buildSecret = function(walletId, walletPrivKey, network) {
  if (lodash.isString(walletPrivKey)) {
    walletPrivKey = PrivateKey.fromString(walletPrivKey);
  }
