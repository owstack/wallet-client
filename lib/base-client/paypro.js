...
PayPro.get = function(opts, cb) {
  ...
  http(opts, function(err, dataBuffer) {
    ...
    var addr = new this.ctx.Address.fromScript(new this.ctx.Script(scriptBuf), network);
...
PayPro.send = function(opts, cb) {
  ...
  var payment = createPayment(opts.merchant_data, opts.rawTx, opts.refundAddr, opts.amountSat);
...
function getPayProRefundOutputs(addrStr, amount) {
  amount = amount.toString(10);

  var output = new PaymentProtocol.Output();
  var addr = new this.ctx.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = this.ctx.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = this.ctx.Script.buildScriptHashOut(addr);
  } else {
...
function createPayment(merchant_data, rawTx, refundAddr, amountSat) {
  ...
  var refund_outputs = getPayProRefundOutputs(refundAddr, amountSat);
  if (refund_outputs) {
...
function getHttp(opts) {
  ...
  return (env == "node") ? nodeRequest : http = browserRequest;;
};
